...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_fullname = filter_var($_POST['user_fullname'], FILTER_SANITIZE_SPECIAL_CHARS);
    $email = filter_var($_POST['email'], FILTER_SANITIZE_EMAIL);
    $password = $_POST['password'];
    $confirmPassword = $_POST['userConfirmPassword'];

    if (empty($registerUserMessage) && !$userController->passwordMatch($password, $confirmPassword)) {
        $registerUserMessage = 'As senhas estão em divergência.';
    }

    if (empty($registerUserMessage) && !$userController->passwordValidation($password)) {
        $registerUserMessage = "A sua senha deve conter:\n- 1 caractere maiúsculo\n- Números\n- Caracteres especiais\n- Entre 8 e 32 caracteres.";
    }

    if (empty($registerUserMessage) && $userController->checkUserByEmail($email)) {
        $registerUserMessage = 'Já existe um usuário cadastrado com esse endereço de e-mail.';
    }

    if (empty($registerUserMessage)) {
        if ($userController->createUser($user_fullname, $email, $password)) {
            header('Location: ../index.php');
            exit();
        }
        $registerUserMessage = 'Erro ao registrar informações. Verifique os dados e tente novamente.';
    }
}
?>
...
<body class="bgLinearGradient">
    ...
    <main class="d-flex justify-content-center align-items-center flex-column h-100">
        <form method="POST" class="bg-light p-5 rounded-4 w-25" enctype="multipart/form-data">
            ...
            <div class="mb-3 position-relative">
                ...
                <input type="file" name="profilePhoto" id="profilePhoto" accept="image/png, image/jpeg">
            </div>
            ...
            <div class="mb-3 position-relative">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="user_fullname" class="form-control border-start-0 inputPlaceholder p-2"
                        id="userName" placeholder="Seu nome completo" aria-describedby="userName" required>
                </div>
            </div>
    ...
    <script>
        <?php if (!empty($registerUserMessage)): ?>
            Toastify({
                text: <?= json_encode(strip_tags($registerUserMessage)); ?>,
                duration: 4000,
                gravity: "bottom",
                position: "right",
                close: true,
